import { toEthiopian, toGregorian, isLeapYear } from './utils/ethiopianCalendar.js';
...
function testConversion() {
  ...
  dates.forEach(gDate => {
    ...
    // Check if the roundTrip is exactly the same date as gDate
    // Match floor of components to avoid time difference issues
    const match = gDate.getFullYear() === roundTrip.getFullYear() && 
                  gDate.getMonth() === roundTrip.getMonth() && 
                  gDate.getDate() === roundTrip.getDate();
